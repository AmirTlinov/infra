...
NEW_TOOL_FLOW = The end-to-end steps to add or change a canonical CLI-backed tool safely.
...
## Layout ([SRC_LAYOUT])
- `src/cli.rs`: public CLI router and JSON envelope emitter
- `src/tooling/`: canonical tool names, contracts, and effect rules
...
- `src/managers/`: tool handlers ([MANAGER]) implementing the operator surface
...
3) Wire it in `src/app.rs` handlers map and update `src/tooling/names.rs` / `tool_contracts.json` if aliases or contracts change.
4) Add or extend focused tests, then run the [GATE] (`./tools/gate`).
